...
function activateSniperItem(item, angle) {
    ...
    console.log('🎯 activateSniperItem:', item.name, '| Type:', item.type, '| Mode:', item.mode, '| multiBarrel:', item.multiBarrel);
    // ⭐ SCYTHE — CHÉM ĐẶC BIỆT (ĐẦU TIÊN!)
    if (item.id === 'collapsed_scythe') {
        if (typeof fireScytheSlash === 'function') {
            fireScytheSlash(angle, item.damage, item);
            return;
        }
    }
    // ========================================================
    ...
    if (item.type === 'weapon') {
        ...
        if (item.multiBarrel) {
            ...
            if (typeof fireShotgunSniperBullet === 'function') {
                fireShotgunSniperBullet(angle, item.damage, item);
            } else {
        ...
        // ⭐ SNIPER MODE
        if (item.mode === 'sniper') {
            console.log('🎯 Bắn sniper | Damage:', item.damage, '| Pierce:', item.pierce);
            if (typeof fireSniperBullet === 'function') {
                fireSniperBullet(angle, item.damage, item.pierce || 0);
            } else {
        ...
        if (item.mode === 'auto') {
            ...
            if (typeof fireAutoBullet === 'function') {
                fireAutoBullet(angle, item.damage);
            } else {
